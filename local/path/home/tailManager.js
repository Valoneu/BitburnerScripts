    ...
    async function enforceSnap() {
        for (const p of ns.ps()) {
            ...
            if (script && script.tailProperties) {
                ...
                const x = snapToGrid(props.x);
                const y = snapToGrid(props.y);
                const w = snapToGrid(props.width);
                const h = snapToGrid(props.height);
                if (x !== props.x || y !== props.y) {
                    ns.ui.moveTail(x, y, p.pid);
                }
                if (w !== props.width || h !== props.height) {
                    ns.ui.resizeTail(w, h, p.pid);
                }
            }
        }
    }
