...
async function prepServer(ns, target, hosts) {
	...
	const wThreads = Math.ceil((Math.max(1, Math.ceil((ns.getServerSecurityLevel(target) - ns.getServerMinSecurityLevel(target)) / ns.weakenAnalyze(1)))) / (BATCH_LIMIT / 1000));
	const gThreads = Math.ceil((Math.max(1, Math.ceil(ns.growthAnalyze(target, ns.getServerMaxMoney(target) / Math.max(1, ns.getServerMoneyAvailable(target)))))) / (BATCH_LIMIT / 1000));
	const wThreadsForGrow = (Math.ceil(ns.growthAnalyzeSecurity(gThreads) / ns.weakenAnalyze(1)));
	const batchRam = ns.getScriptRam(WEAK) * wThreads + ns.getScriptRam(GROW) * gThreads + ns.getScriptRam(WEAK) * wThreadsForGrow;
...
async function shootBatch(ns, target, hosts) {
	...
	const weakenTime = ns.getWeakenTime(target);
	const hackThreads = Math.max(1, Math.floor(ns.hackAnalyzeThreads(target, ns.getServerMaxMoney(target) * HACK_PERCENT)));
	const weaken1Threads = Math.ceil(ns.hackAnalyzeSecurity(hackThreads) / ns.weakenAnalyze(1));
	const growThreads = Math.ceil(ns.growthAnalyze(target, 1 / (1 - HACK_PERCENT)) * OVERFILL);
	const weaken2Threads = Math.ceil((ns.growthAnalyzeSecurity(growThreads) / ns.weakenAnalyze(1)) * OVERFILL);
	const batchRam = ns.getScriptRam(HACK) * hackThreads + ns.getScriptRam(WEAK) * weaken1Threads + ns.getScriptRam(GROW) * growThreads + ns.getScriptRam(WEAK) * weaken2Threads;
