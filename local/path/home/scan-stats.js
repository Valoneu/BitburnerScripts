...
export async function main(ns) {
	...
	while (true) {
		...
		ns.printf("-".repeat(topbar.length));
		ns.printf(topbar);
		ns.printf("-".repeat(topbar.length));
		...
		for (const server of servers) {
			...
			ns.printf(`${server.padEnd(20)} | ` + `${colorText(formatMoney(money).padStart(6), money, moneyMax)} / ${colorText(formatMoney(moneyMax).padEnd(7), moneyMax, ns.getServerMaxMoney(servers[0]))} | ` + `${colorTextInv(sec.toFixed(1).padStart(5), sec, 100 - secMin)} / ${colorTextInv(secMin.toFixed(1).padEnd(5), secMin, ns.getServerMinSecurityLevel(servers[0]))} | ` + `${colorTextInv(lvlneeded.toFixed(0).padStart(4), lvlneeded, ns.getPlayer().skills.hacking)} | ` + `${colorText(score.toFixed(0).padStart(5), score, getScore(ns, servers[0]))} |` + `${colorTextInv(time.toFixed(1).padStart(7), time, 600)} | `);
		}
		await ns.sleep(1000);
	}
}
